...
def index_exists(vsc, endpoint_name, index_full_name):
    try:
        ...
        if 'RESOURCE_DOES_NOT_EXIST' not in str(e):
            logger.error(f'Unexpected error describing the index. This could be a permission '
                         f'issue.')
            raise e
    return False
...
def endpoint_exists(vsc, vs_endpoint_name):
  try:
    ...
    if "REQUEST_LIMIT_EXCEEDED" in str(e):
      logger.info("WARN: couldn't get endpoint status due to REQUEST_LIMIT_EXCEEDED error. The demo will consider it exists")
      return True
    else:
      raise e
...
def wait_for_vs_endpoint_to_be_ready(vsc, vs_endpoint_name):
  for i in range(180):
    try:
      ...
      #Temp fix for potential REQUEST_LIMIT_EXCEEDED issue
      if "REQUEST_LIMIT_EXCEEDED" in str(e):
        logger.info("WARN: couldn't get endpoint status due to REQUEST_LIMIT_EXCEEDED error. Please manually check your endpoint status")
        return
      else:
        raise e
    ...
    if "ONLINE" in status:
      ...
      if i % 20 == 0:
        logger.info(f"Waiting for endpoint to be ready, this can take a few min... {endpoint}")
      time.sleep(10)
    else:
...
def wait_for_index_to_be_ready(vsc, vs_endpoint_name, index_name):
  for i in range(180):
    ...
    if "UNKNOWN" in status:
      logger.info(f"Can't get the status - will assume index is ready {idx} - url: {url}")
      return
    elif "PROVISIONING" in status:
      if i % 40 == 0: logger.info(f"Waiting for index to be ready, this can take a few min... {index_status} - pipeline url:{url}")
      time.sleep(10)
    else:
